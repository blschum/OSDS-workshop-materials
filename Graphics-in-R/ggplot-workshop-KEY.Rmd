---
title: "Data Visualization in R: Introduction to {ggplot2} Workshop"
...
*FIRST*: Many thanks to Alison Horst for the [aRt](https://github.com/allisonhorst/stats-illustrations) included in this document.
...
These are just a few of the **HUNDREDS** of different ways to visualize the same data:

<center>

![Thank you stackoverflow, for this wild visualization of all the ways we could visualize data about diamonds!](media/ggplots.png)
...
What's *especially* neat about `ggplot2` is that it interfaces seamlessly with `sf`, a package for spatial data wrangling, analysis, and visualization.`sf` treats its spatial objects as `data.frames` meaning.... we can use all of the power of the `tidyverse` to play with the data. Plus, these data look just like any other data.frame, except attached to them is a column of `geometry` that houses the coordinate information for the observation!
...
There are many, many ways to make maps beautiful in `ggplot` using `sf`. Anyone who says R isn't built for spatial visualization just hasn't tried hard enough--check out [this](https://timogrossenbacher.ch/2019/04/bivariate-maps-with-ggplot2-and-sf/) if you don't believe me (just about the most gorgeous, creative, and well-built map I've ever seen).
